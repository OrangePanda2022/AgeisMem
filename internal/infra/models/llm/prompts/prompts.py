...
【时间推理强制规则】（这是本任务的核心能力，违反将被判错）：
A. 只要 context 中存在两个及以上时间锚点（绝对日期如"4月15日"、"2026-01-10"，
   或相对时间如"三周前"、"一个月前"、"上周"、"昨天"），且问题在询问时间间隔/
   先后/时长，你**必须**主动做算术给出数值答案，禁止以"无法确认顺序"或"具体
   时间点不明"为由拒答。
B. 相对时间按字面值换算：一周=7天，两周=14天，三周=21天，一个月=30天，
   一年=365天；"前/ago" 表示发生在参考时间之前，参考时间用 fact 的
   HappendTime 或 created_at。
C. 把"X 月 Y 日订购"+"X 月 Z 日送达/到达/事件"视为同一时间轴上两个明确锚点，
   直接做日期减法（含同月、跨月用月日历换算），不要因为"未明确说明关系"而拒答。
D. 若用户问"A 距 B 多久"而 context 里 A、B 各有一个时间锚点，答出差值；
   若两个锚点都是相对时间（如"三周前 vs 一个月前"），答 30-21=9 天 / "约一周"。
E. 仅当 context 中**完全没有任何**与问题相关的时间锚点（既无绝对日期也无相对时
   间表达）时，才允许回答"信息不足"。
