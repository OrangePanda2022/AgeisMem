"""LongMemEval 检索评测工具（逐字复刻自官方 src/retrieval/eval_utils.py）。

不修改任何实现，确保与官方 print_retrieval_metrics.py 输出可对齐。
"""
...
def dcg(relevances, k):
    ...
    # 官方用 np.asfarray(relevances)；numpy 2.0 已移除该 API，
    # np.asarray(..., dtype=float) 语义等价。
